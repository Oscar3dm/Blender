...
def blen_read_material(fbx_tmpl, fbx_obj, settings):
    ...
    existing_mat = bpy.data.materials.get(elem_name_utf8)
    if existing_mat is not None:
        return existing_mat

    # Optionally, check an asset library:
    asset_mat = None
    if hasattr(settings, "asset_library_materials"):
        asset_mat = settings.asset_library_materials.get(elem_name_utf8)
    if asset_mat is not None:
        return asset_mat
    ...
    def _():
        ...
        for fbx_uuid, fbx_item in fbx_table_nodes.items():
            ...
            if fbx_obj.id != b'NodeAttribute':
                continue
            if fbx_obj.props[-1] == b'Camera' and import_cameras:
                assert(blen_data is None)
